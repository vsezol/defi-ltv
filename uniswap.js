...
async function getUniswapPositionsForNetwork(networkKey, walletAddress) {
  ...
  return withRpcFallback(networkKey, async (provider) => {
    ...
    // Positions scanned concurrently — each is an independent chain of read-only
    // calls against the same provider.
    const positions = await Promise.all(
      Array.from({ length: count }, async (_, i) => {
        const tokenId = await positionManager.tokenOfOwnerByIndex(walletAddress, i);
        const position = await positionManager.positions(tokenId);
        if (position.liquidity.isZero()) return null;
...
// state for positions it couldn't see this cycle. All networks scanned in parallel.
export async function getUniswapPositionsForWallet(walletAddress) {
  ...
  const perNetwork = await Promise.all(
    NETWORKS.map(async (network) => {
      try {
        return { positions: await getUniswapPositionsForNetwork(network.key, walletAddress) };
      } catch (error) {
        logger.error({ network: network.key, walletAddress, error: error.message }, "Uniswap scan failed");
        return { failure: `uniswap:${network.key}:` };
      }
    })
  );
  for (const r of perNetwork) {
    if (r.positions) positions.push(...r.positions);
